...
colors_IPR <- c("#555555", colors_IPR)
ggthemr_reset()
colortheme_IPR <- define_palette(
  swatch = colors_IPR, # colours for plotting points and bars
  gradient = c(lower = colors_IPR[1L], upper = colors_IPR[2L]), #upper and lower colours for continuous colours
  background = "#EEEEEE" #defining a grey-ish background
)

theme_IPR <- function (base_size = 11, base_family = "")
{
  ...
  theme(line = element_line(colour = "black", size = 0.5, linetype = 1, lineend = "butt"),
        ...
        legend.margin = margin(unit(0.2, "cm")),
