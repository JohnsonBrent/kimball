...
output:
  word_document:
    fig_height: 4
    fig_width: 6
    reference_docx: IPR_styles_reference-01.docx
    toc: yes
    toc_depth: 2
---
...
<center>
[![Survey data on expected mobile phone life.](D:/Dropbox/kimball/vignettes/mobile phone replacement curve.jpg){ width=70% }](https://www.statista.com/statistics/241036/ownership-time-length-of-current-mobile-phone-until-replacement/)
</center>
...
<center>
[![The Kimball density function estimated using survey data on expected mobile phone life.](D:/Dropbox/kimball/vignettes/mobile phone replacement curve (fitted).jpg){ width=70% }](https://www.statista.com/statistics/241036/ownership-time-length-of-current-mobile-phone-until-replacement/)
</center>
...
#pander::pander(smartphone) 
knitr::kable(smartphone)
```


And here's how one might work with these data in practice and apply the kimball distribution to discover a richer story. I start this below by defining a function to put lagged values of total shipments into separate variables/columns which I name shipments1, shipments2, etc. And I save these lagged mobile phone shipment values inside a data frame called `lagged.shipments`. But note that the first variable inside `lagged.shipments` isn't a lagged value per se but is the current period's sales. When it comes to products like mobile phones, there's usually some small fraction of product that gets replaced within the same year they were purchased (perhaps due to breakage or returns). 
...
The result is an estimate of replacement mobile phone sales. Note that in the table below I also calculate replacement sales as a percentage of total sales (which is just replacements / shipments).  And I compute first-time sales by subtraction (or shipments - replacements).   
...
The point of this example is to show how the Kimball distribution allows an easy way to incorporate judgmental AND analytic inputs about a consumer or business products's replacement rate--all in the context of easily understood parameters--the mean and maximum.
